<br>
<div class="alert alert-primary" role="alert">
    <div class="row">
        @foreach ($datos as $dato)
        <div class="col-3">
            <label>Producto: </label>
            <input type="text"  class="form-control" id="pproducto" value="{{$dato->producto}}, {{$dato->unidad}}" readonly>
        </div>
        <div class="col-3">
            <label>Precio: </label>
            <input type="text"  class="form-control" id="pprecio" value="{{$dato->precio}}" >
        </div>
        @endforeach
        <div class="col-3">
            <label>Cantidad: </label>
            <input type="text" class="form-control" id="pcantidad">
        </div>
        <div class="col-3">
            <label>Importe: </label>
            <input type="text"  class="form-control" id="importe">
        </div>
    </div>
</div>
<script type="text/javascript">
    $(document).ready(function () {
        $('#pcantidad').keyup(function () {
            var precio = $('#pprecio').val(); //obtiene el costo del producto
            var cantidad = $('#pcantidad').val(); //obtiene la cantidad
            var importe = parseFloat(precio) * parseFloat(cantidad); //subtotal
            $("#importe").val(importe);
        });        
    });

</script>
